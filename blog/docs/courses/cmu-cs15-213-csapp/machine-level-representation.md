...
- 传递控制：跳转到被调用过程，结束后返回调用点。
- 传递数据：传递参数，并返回结果。
- 分配和释放内存：为局部变量、保存的寄存器等维护栈空间。
...
- `call Label`：调用目标过程
- 效果：将返回地址压入栈中，跳转到目标过程的起始地址
- 返回地址是 `call` 后面那条指令的地址
- 等价于：
...
!!! note "帧指针"
    ...
    在 x86-64 中，`%rbp` 可以作为帧指针使用，但这不是必须的。优化后的代码常省略帧指针，把 `%rbp` 当作普通被调用者保存寄存器使用。
...
栈向低地址增长，所以分配栈帧通常会减小 `%rsp`，释放栈帧则会增大 `%rsp`。

!!! info "栈帧不总是完整出现"

    如果局部变量都能放在寄存器中，且调用参数也不需要额外栈空间，编译器可能不显式分配栈帧。

### IA32 过程调用

IA32 主要通过栈完成过程调用。调用者在自己的栈帧中准备参数，执行 `call` 后，被调用过程再建立自己的栈帧。

典型流程：

- 建立当前过程的栈帧：保存旧 `%ebp`，令 `%ebp` 指向当前栈帧
- 分配局部变量空间：通过减小 `%esp` 预留栈空间
- 准备调用参数：把参数写入栈中，较早的参数放在较低偏移位置
- 执行 `call`：自动压入返回地址，并跳转到被调用过程
- 取得返回值：返回值通常在 `%eax` 中
- 恢复栈帧并返回：使用 `leave` 和 `ret`
...
x86-64 优先使用寄存器传递参数。只有参数太多、需要保存寄存器、需要局部变量地址，或局部变量无法全部放入寄存器时，才会依赖栈帧。

- 前几个参数优先放入寄存器，更多参数才放在栈上
- 优化后的过程可能不显式使用帧指针 `%rbp`
...
- 前 6 个整数或指针参数依次放在 `%rdi`、`%rsi`、`%rdx`、`%rcx`、`%r8`、`%r9`
- `%rbp` 可作为帧指针，也可作为普通被调用者保存寄存器
- 调用者保存：`%rax`、`%rdi`、`%rsi`、`%rdx`、`%rcx`、`%r8`、`%r9`、`%r10`、`%r11`
- 被调用者保存：`%rbx`、`%rbp`、`%r12`、`%r13`、`%r14`、`%r15`

### 缓冲区溢出

缓冲区溢出是指程序向固定大小的缓冲区写入过多数据，覆盖了相邻的内存内容。

在栈帧中，局部数组通常和保存的寄存器、旧帧指针、返回地址放在相邻区域。如果写入越过数组边界，就可能覆盖这些控制信息。
...
上面的代码没有检查输入长度，输入超过 `buf` 大小时就会继续写入后面的栈空间。

**可能后果**

- 覆盖局部变量，导致程序状态错误
- 覆盖保存的寄存器或旧帧指针，导致返回时栈帧恢复错误
- 覆盖返回地址，使 `ret` 跳转到非预期位置

!!! warning "危险函数"

    `gets`、`strcpy`、`strcat`、`sprintf` 等函数如果不检查目标缓冲区大小，就容易造成缓冲区溢出。实际编程中应使用带长度限制的替代写法。

**常见防护**

- 栈随机化：随机化栈地址，增加预测地址的难度
- 栈保护：在栈帧中插入 canary，返回前检查它是否被覆盖
- 限制可执行代码区域：栈不可执行，避免把栈上数据当作代码运行
- 边界检查：使用安全库函数或显式检查输入长度

## 数据

### 数组

数组在内存中连续存放，数组名表示首元素地址。

**一维数组**

设数组 `A` 的起始地址为 $x_A$，元素类型大小为 $L$，则：
...
对于定长数组 `T A[R][C]`，元素大小为 $L$：

$$
\&A[i][j] = x_A + L \cdot (C \cdot i + j)
$$

其中，$C$ 是每行的元素个数，编译器需要知道它才能计算行偏移。
...
- 变长数组
    ...
    - 地址计算需要使用运行期的维度值
    - 数组本身仍然是连续存放的
    - 局部 VLA 的空间通常在运行期通过调整栈指针分配

```c
int A[n][m];
```

上面的数组大小依赖运行期的 `n` 和 `m`。进入对应作用域时，编译器会计算所需字节数并减小 `%rsp` / `%esp`；离开作用域时再恢复栈指针。

!!! note "VLA 的栈空间"

    VLA 的大小不是编译期常量，因此不能简单使用固定偏移访问所有对象。编译器通常会保留基址寄存器或临时寄存器，用运行期维度计算元素地址。

### 结构

结构把不同类型的对象按声明顺序存放在一段连续内存中。
...
结构字段的偏移由字段顺序、字段大小和对齐规则共同决定。编译器可能在字段之间或结构末尾插入填充字节。

**对齐规则**

- 每个字段都有自己的对齐要求 $K$
- 字段起始地址必须是 $K$ 的倍数
- 结构整体的对齐要求等于所有字段对齐要求的最大值
- 结构总大小必须是整体对齐要求的倍数

常见大小和 $K$ 值：

| 类型 | IA32 大小 / $K$ | x86-64 大小 / $K$ |
| ---- | -------------- | ----------------- |
| `char` / `unsigned char` | 1 / 1 | 1 / 1 |
| `short` / `unsigned short` | 2 / 2 | 2 / 2 |
| `int` / `unsigned int` | 4 / 4 | 4 / 4 |
| `long` / `unsigned long` | 4 / 4 | 8 / 8 |
| `long long` / `unsigned long long` | 8 / 4 | 8 / 8 |
| `float` | 4 / 4 | 4 / 4 |
| `double` | 8 / 4 | 8 / 8 |
...
!!! tip "字段顺序会影响结构大小"
    ...
    把对齐要求大的字段放在前面，通常可以减少填充字节。

### 联合

联合 `union` 的所有字段共享同一段内存。
...
- 所有字段的偏移都是 $0$
- 联合大小等于最大字段大小再按对齐要求补齐
- 联合的对齐要求等于所有字段对齐要求的最大值
- 同一时间只能安全地使用最近写入的那个字段

联合常用于节省空间，或用不同类型解释同一段二进制数据。

!!! warning "类型解释"

    通过联合用一种类型写入、再用另一种类型读取，容易依赖实现细节。做底层数据解释时应明确目标平台、字节序和类型大小。
